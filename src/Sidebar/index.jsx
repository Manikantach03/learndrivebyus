...
const SidebarBootstrap = ({ active, setActive }) => {
  ...
  // Sidebar items based on accountType
  const navItems =
    accountType === "User"
      ? [
          { name: userName || "My Dashboard", icon: "bi-person", path: "home" }, // 👈 show username
          { name: "Book Slot", icon: "bi-calendar-check", path: "bookslot" },
          { name: "My Bookings", icon: "bi-journal-check", path: "mybookings" },
          { name: "Nearby School", icon: "bi-geo-alt", path: "nearby" },
        ]
      : [
          { name: "Home", icon: "bi-house", path: "home" },
          { name: "My Students", icon: "bi-people", path: "mystudents" },
          { name: "My Schools", icon: "bi-people", path: "myschools" },
        ];

  return (
    <div
      className={`d-flex flex-column bg-white border-end vh-100 position-sticky top-0 ${
